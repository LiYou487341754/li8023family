...
class AutoCompletPapers():
    def __init__(self, session, courseId, title):
        ...
        courseList = \
            self.get(
                f'https://www.cqooc.com/json/chapters?limit=200&start=1&sortby=selfId&isPublish=1&courseId={self.courseId}&select=id,title,level,selfId,parentId,status,pubClass,isElective&ts={getTs()}',
                headers={
                    'Referer': f'https://www.cqooc.com/learn/mooc/structure?id={self.courseId}',
                }).json()['data']
        Ids = []
        for i in range(len(courseList)):
            parentId = courseList[i]['id']
            if courseList[i]['parentId'] != None:
                Ids.append(parentId)
        Ids = sorted(Ids)
        for parentId in Ids:
            courselist = \
                self.get(
                    f'https://www.cqooc.com/json/mooc/lessons?parentId={parentId}&limit=200&sortby=selfId&reverse=false&ts={getTs()}',
                    headers={
                        'Referer': f'https://www.cqooc.com/learn/mooc/structure?id={self.courseId}',
                    }).json()['data']
            chapterId = courselist[0]['parentId']
            sectionIds = []
            for i in range(len(courselist)):
                sectionId = courselist[i]['id']
                sectionIds.append(sectionId)
            sectionList = sorted(sectionIds)
        try:
    ...
    def saveAnswersFromDue(self, papersIds):
        ...
        for parentId, sectionId in self.sectionList.items():
            print(parentId, sectionId)
            for id_index, id in enumerate(papersIds):
                req_url = f'https://www.cqooc.com/test/api/paper/get?id={id}&ts={getTs()}'

                response = self.get(req_url, headers={
                    'Referer': f'https://www.cqooc.com/learn/mooc/testing/do?tid={id}&id={self.courseId}&sid={sectionId}&cid={parentId}&mid={self.mid}',
                })
    ...
    def saveAnswersFomUser(self, papersIds):
        # 保存已作答题目的答案
    ...
    def getAnswersFromUser(self, paperId):
        # 获取答案（从另一个用户）
        self.cookieXsidUser = log_in() if self.cookieXsidUser is None else self.cookieXsidUser
    ...
    def sendAnswers(self, mode=None):
        ...
        for index, id in enumerate(papersInfo):
            ...
            if response.json().get('code') == 100:
                ...
                print(f"[{index + 1}/{len(papersInfo)}] 得分: {response.json().get('score')} {papersInfo[id]}")
        ...
        while True:
            ...
            if select == '1':
                ...
                if n == '1':
                    ...
                    autoCompletPapers = AutoCompletPapers(self.Session, self.courseId, self.title)
                    autoCompletPapers.saveAnswersFomUser(papersIds)
                else:
                    print("输入有误，请重新输入！")
            elif select == '5':
                pass
    ...
    def startLearnCourse(self):
        ...
        for parentId in Ids:
            ...
            for i in range(len(courselist)):
                ...
                sectionIds.append(sectionId)
            sectionList = sorted(sectionIds)
            print('\r章节进度: {}/{}({:.2f}%) \t当前: {}'.format(index_t + 1, len(Ids),
                                                                 ((float(
                                                                     (index_t + 1) / len(
                                                                         Ids))) * 100),
                                                                 self.courseDes.get(chapterId)))
            index_t += 1
            for index, sectionId in enumerate(sectionList):
                print('\r\t小节进度: %d/%d(%.2f%%)' % (
