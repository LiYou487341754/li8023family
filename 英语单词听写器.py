...
def write_in():  # 写入文件
    ...
    try:
        ...
        for k in range(len(data)):
            write = re.findall(entry1.get(), data[k][0])
            if write != entry1.get() and len(entry1.get()) != 0:
                label3.configure(text="写入成功" + "   " + entry1.get() + "   :  " + entry2.get(), font=("华文新魏", 13),
                                 foreground='red')
                ...
                figures = i_figures()
                figure = j_figure()
            elif write == entry1.get() and len(entry1.get()) != 0:
                label3.configure(text='单词已经记录')
                root.update()
            else:
                label3.configure(text='请输入记录单词')
                root.update()
...
label3 = ttk.Label(frame6, text=" ", font=("华文新魏", 13),
                   foreground='red')
# 布局
frame4.pack()
frame5.pack()
